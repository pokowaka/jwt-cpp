...
TEST(base64_test, quick_fox) {
  std::string inputData =
      "The quick brown fox jumps over the lazy dog and some extr";
  std::string expectedResult = "VGhlIHF1aWNrIGJyb3duIGZveCBqdW1wcyBvdmVyIHRoZSB"
                               "sYXp5IGRvZyBhbmQgc29tZSBleHRy";
  EXPECT_STREQ(expectedResult.c_str(),
               Base64Encode::EncodeUrl(inputData).c_str());
  EXPECT_STREQ(inputData.c_str(),
               Base64Encode::DecodeUrl(expectedResult).c_str());
}

TEST(base64_test, buffer_overflows) {
  std::string str_dec = "VGhlIHF1aWNrIGJyb3duIGZveCBqdW1wcyBvdmVyIHRoZSBsYXp5IG"
                        "RvZyBhbmQgc29tZSBleHRy";
  char buffer[4096];

  for (size_t num_dec = 6; num_dec < str_dec.size(); num_dec++) {
    for (size_t num_buffer = 0;
         num_buffer < Base64Encode::DecodeBytesNeeded(num_dec) / 2;
         num_buffer++) {
      EXPECT_EQ(1, Base64Encode::DecodeUrl(str_dec.c_str(), num_dec, buffer,
                                           &num_buffer));
    }
  }
}
...
TEST(base64_test, buffer_underflows) {
  ...
  for (size_t num_enc = 6; num_enc < str_enc.size(); num_enc++) {
    for (size_t num_buffer = 0;
         num_buffer < Base64Encode::EncodeBytesNeeded(num_enc) / 2;
         num_buffer++) {
      EXPECT_EQ(1, Base64Encode::EncodeUrl(str_enc.c_str(), num_enc, buffer,
                                           &num_buffer));
    }
  }
}
...
TEST(base64_test, invers) {
  ...
  ASSERT_STREQ(res.c_str(),
               Base64Encode::EncodeUrl(Base64Encode::DecodeUrl(res)).c_str());
}
...
TEST(base64_test, perf_encode_cplus) {
  ...
  //  => #<Benchmark::Tms:0x4310d43 @stime=0.00999999999999801, @label="",
  //  @cstime=0.0, @real=2.2860000133514404, @total=2.329999999999991,
  //  @cutime=0.0, @utime=2.319999999999993>
...
TEST(base64_test, perf_decode_c) {
  //#<Benchmark::Tms:0x47d9a273 @stime=0.010000000000001563, @label="",
  //@cstime=0.0, @real=1.4619998931884766, @total=1.5199999999999925,
  //@cutime=0.0, @utime=1.509999999999991>  vs 328ms (5x faster).
