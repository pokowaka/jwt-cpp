...
TEST(parse_test, proper_hmac) {
  for (auto &hmac : hmacs) {
    ...
    EXPECT_STREQ(hmac, valid->algorithm().c_str());
  }
}
...
TEST(parse_test, can_use_validator) {
  std::string json = R"({ "HS256" : { "secret" : "secret" } })";
...
TEST(parse_test, proper_rsa) {
  for (auto &r : rs) {
    ...
    json << "{ \"" << r
         << "\" : { \"public\" : \""
            "-----BEGIN PUBLIC KEY-----\\n"
            "MFswDQYJKoZIhvcNAQEBBQADSgAwRwJAdTI8v0w96101cfpvMHPruu1kqViskO"
            "b"
            "b\\n"
    ...
    validator_ptr valid(MessageValidatorFactory::Build(json.str()));
    EXPECT_NE(nullptr, valid.get());
    EXPECT_STREQ(r, valid->algorithm().c_str());
  }
}
  ...
  for (auto &r : rs) {
    ...
    json << "{ \"" << r
         << "\" : { \"public\" : \""
            ...
            "ZqvglxOMtUzpdO7USdlFmyOEjtH1tioll9EAg6DMs0QrLgBj7U0XHRHeJcRrbY"
            "x"
            "m\\n"
            "HqtmtRxjEmLBpClJoYaJ2fEdeaVcV5D1+"
            "kWMIRLM1q3RNafb1Q62nwSyojgX09/"
            "X\\n"
            "+lWtkuX4NPwnn5NW13uhLyO96bANWMzPhYewwCsY7s7HCscNEhVTLQF0UmtYMg"
            "p"
            "n\\n"
            ...
            "GNETAL2CRXDThUdkIMMF3AcsiuZY7O4dasOPTyxffPTjhaEX6rlwjHdd0EjEjC"
            "7"
            "T\\n"
            ...
            "azX+wWSBIcEW\\n"
            "Lbk6MGCt2z7mWGla4x7ToxhYWBhRdDxZ0R3VzG05e1Yjn1q2U5uxsSdBAPAISg"
            "e"
            "D\\n"
            ...
            "CObCMKwvI\\n"
            "kMisBwFZEyS1oiV2Lt//8tLDnrlvxQrKQLmEzI5kCbuh3EUiG/"
            "tMF4VmKB4+JR/"
            "2\\n"
            "TNsHCqeNuKmVjy+"
            ...
            "AMW5JqgvIk\\n"
            "qfw3pW1HIMxctzyVipEkg0tQa5XeQf4sEguIQ4Os8eS4SE2QFVr8MWoz5czMOq"
            "p"
            "F\\n"
...
TEST(parse_test, improper_rsa) {
  for (auto &r : rs) {
    ...
    json << "{ \"" << r
         << "\" : { \"public\" : \""
            "-----BEGIN PUBLIC KEY-----\\n"
            "MFswDQYJK3ZIhvcNAQEBBQADSgAwRwJAdTI8v0w96101cfpvMHPruu1kqViskO"
            "b"
            "b\\n"
            "Nnmy3FmhiJX0o5KNOKOEWKnTUoGfM7TbfV5WGRcXW37W4cBUQ2dLWwIDAQAB\\"
            "n"
...
TEST(parse_test, proper_rsa_from_file) {
  ...
  for (auto &r : rs) {
    std::ostringstream json;
    json << "{ \"" << r
         << R"(" : { "public" : { "fromfile" : "/tmp/test.key" } } })";
    validator_ptr valid(MessageValidatorFactory::Build(json.str()));
...
TEST(parse_test, accepts_multiple_types) {
  ...
  std::string json = "{ \"kid\" : { "
                     ...
                     "} }";
  ASSERT_NO_THROW(
      std::unique_ptr<MessageValidator>(MessageValidatorFactory::Build(json)));
}
...
TEST(parse_test, non_existing) {
  std::string json = R"({ "HS253" : { "secret" : "safe!" } })";
  ASSERT_THROW(MessageValidatorFactory::Build(json), std::logic_error);
}

TEST(parse_test, too_many_properties) {
  std::string json =
      R"({ "HS256" : { "secret" : "safe!" }, "FOO" : "BAR" })";
  ASSERT_THROW(MessageValidatorFactory::Build(json), std::logic_error);
}
...
TEST(parse_signer, non_existing_signer) {
  std::string json = R"({ "HS252" : { "secret" : "safe!" }})";
  ASSERT_THROW(MessageValidatorFactory::BuildSigner(json), std::logic_error);
}

TEST(parse_test, non_secret) {
  std::string json = R"({ "HS256" : { "without_secret" : "safe!" } })";
  ASSERT_THROW(MessageValidatorFactory::Build(json), std::logic_error);
}
