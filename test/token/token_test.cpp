...
TEST(token_test, valid) {
    ...
    std::unique_ptr<Token> token(Token::parse(tokenstr.c_str(), tokenstr.size()));
    ClaimSet *header = token->header_claims();

    EXPECT_NE(nullptr, token);
    EXPECT_EQ(false, token->IsEncrypted());
    EXPECT_STREQ("JWT", header->Get("typ").c_str());
}
...
TEST(token_test, payload_deserialize) {
    ...
    ClaimSet* payload = token->payload_claims();
    EXPECT_STREQ("1234567890", payload->Get("sub").c_str());
    EXPECT_STREQ("true", payload->Get("admin").c_str());
    EXPECT_STREQ("John Doe", payload->Get("name").c_str());
}
...
TEST(token_test, keymaster_token) {
    ...
    EXPECT_STREQ("RSA1_5", header->Get("alg").c_str());
    EXPECT_STREQ("A256CBC", header->Get("enc").c_str());
    EXPECT_STREQ("42", header->Get("kid").c_str());
